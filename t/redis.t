...
# Isolate tests
my $minion = Minion->new('Redis');
$minion->backend->name('minion_test');
$minion->reset;
...
$minion->backend->redis->hset("minion.minion_test.worker.$id",
  notified => time - ($minion->missing_after + 1));
$minion->repair;
...
my $finished
  = $minion->backend->redis->hget("minion.minion_test.job.$id2", 'finished');
$minion->backend->redis->hset("minion.minion_test.job.$id2",
  finished => $finished - ($minion->remove_after + 1));
$finished
  = $minion->backend->redis->hget("minion.minion_test.job.$id2", 'finished');
$finished
  = $minion->backend->redis->hget("minion.minion_test.job.$id3", 'finished');
$minion->backend->redis->hset("minion.minion_test.job.$id3",
  finished => $finished - ($minion->remove_after + 1));
$worker->unregister;
$minion->repair;
...
# Reset
$minion->reset->repair;
ok !$minion->backend->redis->scard('minion.minion_test.default.jobs'), 'no jobs';
ok !$minion->backend->redis->scard('minion.minion_test.workers'), 'no workers';
...
my $new_delaled = time - 86400;
$minion->backend->redis->hset("minion.minion_test.job.$id", delayed => $new_delaled);
$minion->backend->redis->zadd('minion.minion_test.delayed', $new_delaled, "default:$id:0");
$job = $worker->dequeue(0);
is $job->id, $id, 'right id';
